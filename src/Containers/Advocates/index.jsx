...
import Card from './AdvocatesCard'

import {advo} from './data.js'
    ...
    <Wrapper {...props}>
      <Box m={1} style={{ width: '100%' }}>
        <Grid item xs={12} sm={12} md={12} lg={12} style={{ margin: 'auto', textAlign: 'center' }} >
          ...
          <Typography variant='h3' className={classes.title}>
            View Our Advocates Below <span role='img' aria-labelledby='finger-down'>👇</span>
          </Typography>
        </Grid>
      </Box>
      <Grid item xs={12} sm={12} md={10} lg={10}>
        <Grid container justifyContent='center'>
          {
            advo.map((info, index) => <Card image={info.image} link={info.pdf} key={index} />)
          }
        </Grid>
      </Grid>
    </Wrapper>
