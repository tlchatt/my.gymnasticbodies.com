...
export const continutePreviosLevel = (levelId) => (dispatch, getState) => {
  ...
  // Guided-plan level selection is Neon for all users — legacy guided-plan state was
  // seeded into user_logs section 'levels' on 2026-08-03.
  dispatch({
...
export const setLevelPath = (leveld, isCallback = false, workoutOrPlanId = 0) => (dispatch, getState) => {
  ...
  // Guided-plan level selection is Neon for all users. setLevelPathNew dispatches
  // SET_USER_LEVEL itself, so the reducer update is not lost with the AWS branch.
...
export const getLevelPlanNew = (type) => (dispatch, getState) => {//userLevel: "Advanced One"
  ...
  let userData = {
    // Legacy sessions carry the AWS integer in UserId — Neon routes must get the UUID or
    // the row is written under a key nothing reads back.
    userId: neonIdOf(state),
  ...
  Axios.get(NEWAPI + '/api/user/userStatus', {
    ...
    .then(res => {
      ...
      let keys = Object.keys(workoutSchedule);
      console.log("workoutSchedule later in else:", workoutSchedule)
      //keys is the date
...
export const ManageDificultyNew = (workoutIndex, dateKey, dateKeyIndex, exerciseId, type, section, step) => (dispatch, getState) => {
  ...
  */
  ...
  console.log("data[dateKey][workoutIndex]", data[dateKey][workoutIndex])
  console.log("section:", section)
  const programAtLevel = data[dateKey][workoutIndex].workout[`LEVEL ${levelId}`][section].find((item) => item.exerciseId === exerciseId);
  ...
  console.log("programAtLevel before", programAtLevel)
  ...
  console.log("programAtLevel after", programAtLevel)
  console.log("data at last:", data)
  const programAtLevelTest = data[dateKey][workoutIndex].workout[`LEVEL ${levelId}`][section][0]
  console.log("programAtLevelTest:", programAtLevelTest)
  ...
  dispatch(showToast('Step Updated, Leveled ' + type + ' Successfully', 'success'))

  allProgramNewData = [...allProgramNewData, ...programData]
  console.log("allProgramNewData:", allProgramNewData)//workout data
  console.log("userSchedule:", userSchedule)//all the days data
  dispatch({
    type: actionTypes.GET_WORKOUT,
    payload: {
...
export const LogLegacyNew = (exerciseId, mobilityStatus, autoProg, steps, logList, dateKeyIndex, dateKey, workoutIndex) => (dispatch, getState) => {
  ...
  data[dateKey][workoutIndex].workout[`LEVEL ${levelId}`][section][0].isLogged = false
...
export const GetAllWorkoutInfo = (dateKey, workoutIndex, dateKeyIndex) => (dispatch, getState) => {
  ...
  Axios({method:'get',url:`${NEWAPI}/api/user/workout/byo/program?userId=${encodeURIComponent((state.login.neonUserId||localStorage.getItem('neonUserId')))}&courseId=${legacyNameToId[courseName]}&view=edit`,headers:{'Authorization':`Bearer ${webToken}`}})
    .then(res => {
      ...
      console.log("inside getAllWoroutInfo, allOrderedData:", allOrderedData)
      let responseData
      //check localstorage
      let progressions = localStorage.getItem('progressions')
      responseData = progressions ? JSON.parse(progressions) : null
      console.log("responseData 1:", responseData)
      if (responseData === null) {//fetch from db
        let userData = {
          userId: userid,
          userScheduleDate: dateKey
        }
        ...
        Axios.get(NEWAPI + '/api/user/log', {
          ...
          .then(res => {
            ...
            logs.forEach(log => {
              if (log.userScheduleDate === dateKey && Object.entries(log?.progressions).length > 0) {
                responseData = log?.progressions?.progressions
              } else {//else customObject
                if (courseName == "Upper Body") {
                  responseData = { ...defaultUpperBodyProgressions }
                }
                if (courseName === "Core") {
                  responseData = { ...defaultCoreProgressions }
                }
                if (courseName == "Lower Body") {
                  responseData = { ...defaultLowerBodyProgressions }
                }
              }
            })
